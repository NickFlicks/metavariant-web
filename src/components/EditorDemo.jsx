  ...
  return (
    <div className="rounded-lg border border-white/60 bg-white/60 p-4">
      ...
      <div className="mt-3 min-h-[92px]">
        {status === "idle" && (
          <div className="flex h-[92px] items-center justify-center rounded-lg border border-dashed border-slate-300 text-[11px] text-ink-muted">
            Search, or upload a brand-new file directly.
          </div>
        )}
        {status === "loading" && (
          <div className="flex h-[92px] items-center justify-center gap-2 rounded-lg border border-slate-200 bg-white text-[11px] text-ink-muted">
  ...
  return (
    <div className="rounded-lg border border-white/60 bg-white/60 p-4">
          ...
          return (
            <button
              ...
              className={`flex h-7 w-7 items-center justify-center rounded-md transition-colors ${
                active ? "bg-brand-100 text-brand-700" : "text-ink-muted hover:bg-slate-100"
              }`}
            >
      ...
      <div className="mt-3 rounded-lg border border-slate-200 bg-white p-3 text-xs leading-relaxed text-ink-secondary">
        <span className={marks.has("bold") ? "font-bold text-ink" : ""}>
          ...
          <div key={f.id} className="relative rounded-lg border border-slate-200 bg-white p-3">
            ...
            <div
              className={`mt-2 grid transition-all duration-300 ease-out ${
                openId === f.id ? "grid-rows-[1fr] opacity-100" : "grid-rows-[0fr] opacity-0"
              }`}
            >
              <div className="overflow-hidden">
                <p className="rounded-md bg-brand-50/70 p-2 text-[11px] leading-relaxed text-brand-800">
                  {f.copy}
                </p>
              </div>
            </div>
  ...
  return (
    <div className={`glass glass-hover rounded-xl2 p-5 sm:p-6 ${className}`}>
      <div className="flex flex-wrap items-center justify-between gap-3">
        ...
        <div className="inline-flex flex-wrap gap-1 rounded-full border border-white/60 bg-white/50 p-0.5 text-xs font-semibold backdrop-blur">
          {TABS.map((t) => (
            <button
              key={t.id}
              type="button"
              ...
              className={`rounded-full px-3 py-1.5 transition-colors ${
                activeId === t.id ? "bg-ink text-white shadow-soft" : "text-ink-muted"
              }`}
            >
              {t.label}
            </button>
      ...
      <div key={activeId} className="animate-pop mt-4">
        <Active />
      </div>
    </div>
  );
